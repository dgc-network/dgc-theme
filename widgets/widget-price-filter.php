...
if (!class_exists('DGC_Price_Filter_Widget')) {
	class DGC_Price_Filter_Widget extends WP_Widget {
		...
		public function widget($args, $instance) {
			...
			if ($display_type === 'slider') {
				$html .= '<div class="dgc-price-filter-wrapper">';
					...
					$html .= '<div class="slider-values">';
						$html .= '<p>' . __('Min Price', 'textdomain') . ': <span class="wcapf-slider-value" id="wcapf-noui-slider-value-min"></span></p>';
						$html .= '<p>' . __('Max Price', 'textdomain') . ': <span class="wcapf-slider-value" id="wcapf-noui-slider-value-max"></span></p>';
					$html .= '</div>';
				$html .= '</div>';
			}
			...
			if ($display_type === 'slider') {
				$widget_class = 'woocommerce dgc-price-filter-widget';
			} else {
				$widget_class = 'woocommerce dgc-price-filter-widget wcapf-ajax-term-filter';
			}
...
// register widget
if (!function_exists('dgc_price_filter_widget')) {
	function dgc_price_filter_widget() {
		register_widget('DGC_Price_Filter_Widget');
	}
	add_action('widgets_init', 'dgc_price_filter_widget');
}
