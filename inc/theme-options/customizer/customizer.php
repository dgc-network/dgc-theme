...
class dgc_theme_options_customizer {
	...
	public function settings_fields () {
		global $dgc__options;
		$this->sections = $dgc__options->settings_fields();
	}

	public function register_settings ($wp_customize) {
		global $dgc__options;
		$opt_name = $dgc__options->args['opt_name'];	
		...
		if ( is_array($this->sections)) {
			foreach ($this->sections as $option ) {
				...
				foreach ( $option['fields'] as $field) {
					...
					if ($f_type == 'checkbox') {
						$sanitize =  'dgc__sanitize_checkbox';
					}	
					else {
						$sanitize = '';
					}							
					if (!empty($field['fields'])) {
						foreach ($field['fields'] as $key=>$group) {
							...
							$wp_customize->add_setting(esc_attr($id), array(
							 ...
							 switch ( $g_type ) {
								case 'checkbox':
									$wp_customize->add_control(new dgc_mize_Checkbox_Control($wp_customize, esc_attr($id), array(
										'type'              => 'checkbox',
										'priority'          => $priority,
										'section'           => $section,
										'label'             => $g_label,
								...
								break;
								case 'text':
									$wp_customize->add_control(new dgc_mize_Text_Control($wp_customize, esc_attr($id), array(
										'type'              => 'text',
										'priority'          => $priority,
										'section'           => $section,
										'label'             => $g_label,
								...
								break;
								case 'select':
									$wp_customize->add_control(new dgc_mize_Select_Control($wp_customize, esc_attr($id), array(
										'type'              => 'select',
										'priority'          => $priority,
										'section'           => $section,
										'label'             => $g_label,
								...
								break;
								case 'textarea':
									$wp_customize->add_control(new dgc_mize_Textarea_Control($wp_customize, esc_attr($id), array(
										'priority'          => $priority,
										'section'           => $section,
										'label'             => $g_label,
										'description'       => $g_description,
					...
					switch ($f_type) {
						...
						break;
						case 'font':
							$wp_customize->add_control(new dgc_mize_Font_Control($wp_customize, esc_attr($id), array(
								'type'              => 'select',
								'priority'          => $priority,
								'section'           => $section,
								'label'             => $f_label,
...
global $dgc__options_customizer;	
$dgc__options_customizer = new dgc_tdgc_ons_customizer();
